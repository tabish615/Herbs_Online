...
class AllHoneys extends Component {
    constructor() {
        super();
        this.state = {
            rows : honeyItems,
    ...
    detail(data) {
        ...
        this.props.history.push(`/honeys/product/${name}`, {honeyData : data})
    }

    onSearchHandler = (e) => {
        this.setState({ rows: honeyItems.filter(str => str.Roman_Name.toLowerCase().includes(e.target.value.toLowerCase()))})
      }

    render() {
        return (
            <Grid className="mainDiv" container style={{ padding: "0", margin: "0", display: "flex", justifyContent: "space-around", marginTop : "10%", minHeight : "77vh" }}>
